...
3. Upgrade GLeadGenerator.Integration.Contract\GLeadGenerator.Integration.Contract.csproj.
4. Upgrade GLeadGenerator.Integration.Service\GLeadGenerator.Integration.Service.csproj.
5. Upgrade GLeadGenerator.Integration.Api\GLeadGenerator.Integration.Api.csproj.
6. Upgrade GLeadGenerator.Infrastructure\GLeadGenerator.Infrastructure.csproj.
...
#### GLeadGenerator.Integration.Contract\GLeadGenerator.Integration.Contract.csproj modifications

Project properties changes:
  - Target framework should be changed from `net7.0` to `net10.0`.

Feature upgrades:
  - Rename namespaces and using directives from `GLeadGenerator` to `GLeadGenerator` across all contract DTOs.

Other changes:
  - Rebuild to confirm DTO API surface remains compatible with consuming services.

#### GLeadGenerator.Integration.Service\GLeadGenerator.Integration.Service.csproj modifications
...
Feature upgrades:
  - Rename namespaces/usings from `GLeadGenerator` to `GLeadGenerator` across background services and handlers.

Other changes:
  - Rebuild service to catch API changes introduced by .NET 10.

#### GLeadGenerator.Integration.Api\GLeadGenerator.Integration.Api.csproj modifications
...
Feature upgrades:
  ...
  - Rename namespaces/usings from `GLeadGenerator` to `GLeadGenerator` throughout the API (controllers, middleware, Program.cs).
...
#### GLeadGenerator.Infrastructure\GLeadGenerator.Infrastructure.csproj modifications
...
Feature upgrades:
  ...
  - Rename namespaces/usings from `GLeadGenerator` to `GLeadGenerator` throughout middleware, handlers, and DI modules.
